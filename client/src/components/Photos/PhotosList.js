    ...
    return(
        <div className='scroll-smooth hover:scroll-auto'>
            ...
            <div className='grid md:grid-cols-4 lgLgrid-cols-6 xl:grid-cols-7 gap-4 m-3 max-w-7xl mx-auto px-4'>
               ...
               {isLoading ? <PhotoSkeleton item={20}/> : response.map((data, key) => <Photo item={20} key={key} data={data} profilePhoto={props.profilePhoto} setProfilePhoto={props.setProfilePhoto} setPhotoReady={props.setPhotoReady} photoReady={props.photoReady}/>)}
            </div>
        </div>
    )
}
